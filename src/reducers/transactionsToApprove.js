...
// -- Constants --------------------------------------- //
const WALLETCONNECT_UPDATE_TRANSACTIONS_TO_APPROVE =
  'wallet/WALLETCONNECT_UPDATE_TRANSACTIONS_TO_APPROVE';
...
export const getNativeAmount = (
  ...
  if (prices && prices[nativeCurrency] && prices[nativeCurrency][symbol]) {
    ...
    const displayAmount = convertAssetAmountToDisplaySpecific(
      _nativeAmount,
      prices,
      nativeCurrency
    );
...
const getTransactionDisplayDetails = (
  ...
  if (transaction.data == '0x') {
    ...
    const nativeAmount = getNativeAmount(
      prices,
      nativeCurrency,
      value,
      asset.symbol
...
export const addTransactionsToApprove = transactions => (
  dispatch,
  getState
) => {
  const { transactionsToApprove } = getState().transactionsToApprove;
  ...
  const transactionsWithDisplayDetails = mapValues(
    transactions,
    transactionDetails => {
      const transactionDisplayDetails = getTransactionDisplayDetails(
        transactionDetails.callData,
        accountInfo.assets,
        prices,
        nativeCurrency
      );
