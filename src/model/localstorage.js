...
export const getAccountLocalRequests = async (accountAddress, network) => {
  const accountLocal = await commonStorage.getLocal(
    accountAddress.toLowerCase()
  );
  const requests =
    accountLocal && accountLocal[network] ? accountLocal[network].requests : {};
  const openRequests = pickBy(
    requests,
    request =>
      differenceInMinutes(Date.now(), request.transactionPayload.timestamp) < 60
  );
